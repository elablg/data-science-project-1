...
def test_question_3_query():
    expected = [('USB-C Cable', Decimal('49.95')), ('Wireless Mouse',
                                                    Decimal('199.90')), ('Laptop Stand', Decimal('329.50'))]
    run_common_test(expected, question_3_query)


def test_question_4_query():
    expected = [(3, 'John Doe', 'john.doe@example.com', 'USA', datetime.date(2020, 11, 15)), (2, 'Ayşe Yılmaz', 'ayse.yilmaz@example.com', 'Turkey', datetime.date(2021, 5, 3)), (6, 'Merve Demir', 'merve.d@example.com', 'Turkey', datetime.date(2021, 9, 30)),
                (1, 'Ali Veli', 'ali.veli@example.com', 'Turkey', datetime.date(2022, 1, 10)), (5, 'Carlos Mendez', 'carlos.m@example.com', 'Mexico', datetime.date(2022, 7, 12)), (4, 'Emma Brown', 'emma.b@example.co.uk', 'UK', datetime.date(2023, 2, 21))]
...
def test_question_7_query():
    expected = [('27" Monitor',), ('Gaming Keyboard',), ('Laptop Stand',),
                ('Noise-Cancelling Headphones',), ('USB-C Cable',), ('Wireless Mouse',)]
    run_common_test(expected, question_7_query)


def test_question_8_query():
    expected = [(1, 'ali.veli@example.com'), (2, 'ayse.yilmaz@example.com'), (3,
                                                                              'john.doe@example.com'), (4, 'emma.b@example.co.uk'), (5, 'carlos.m@example.com')]
    run_common_test(expected, question_8_query)


def test_question_9_query():
    expected = [(7, Decimal('49.95')), (4, Decimal('199.90')),
                (1, Decimal('249.90'))]
    run_common_test(expected, question_9_query)


def test_question_10_query():
    expected = [(1, 'Ali Veli', 'ali.veli@example.com', 'Turkey', datetime.date(2022, 1, 10)), (2, 'Ayşe Yılmaz', 'ayse.yilmaz@example.com',
                                                                                                'Turkey', datetime.date(2021, 5, 3)), (6, 'Merve Demir', 'merve.d@example.com', 'Turkey', datetime.date(2021, 9, 30))]
