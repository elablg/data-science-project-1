...
def question_3_query():
    ...
    cursor = connection.cursor()
    cursor.execute(
        'SELECT product_name, price FROM products ORDER BY price LIMIT 3;')
    data = cursor.fetchall()
    cursor.close()
    connection.close()
    return data
...
def question_9_query():
    ...
    cursor = connection.cursor()
    cursor.execute(
        'SELECT order_id, total_amount FROM orders ORDER BY total_amount ASC LIMIT 3;')
    data = cursor.fetchall()
    cursor.close()
    connection.close()
    return data
...
def question_10_query():
    ...
    cursor = connection.cursor()
    cursor.execute(
        "SELECT * FROM customers WHERE country = 'Turkey' ORDER BY customer_name ASC;")
    data = cursor.fetchall()
    cursor.close()
    connection.close()
    return data
